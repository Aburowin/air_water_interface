...
load('/Attached code/PCO4000_DI.mat')
lm_dirlist = dir(['/Attached code/' '*Cam_' '*.b16']);
...
for img = 1 : length(lm_dirlist)
    ...
    temp = im_hp2_lp;
    signal  = im_hp2_lp(2000:end-7, :);
    clear ind
    for ss = 1:size(signal,2)
        clear change_points
        change_points = findchangepts(signal(:,ss),MaxNumChanges=3,Statistic="rms");
        if isempty(change_points)
            change_points = ind(ss-1) - 2000;
        end
        ind(ss) = change_points(end) + 2000;
        temp(1:ind(ss),ss) = 1;
        temp(ind(ss)+1:end,ss) = 0;
    end

    th = mean(im_hp2_lp(2500:end-7, :), 1);
    th = repmat(th, [2672 1]);
    d1 = im_hp2_lp - 1.3 * th;
    % response to yourtemp = d1 > 0;
